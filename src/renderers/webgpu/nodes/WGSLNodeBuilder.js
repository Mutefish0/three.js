...
class WGSLNodeBuilder extends NodeBuilder {
	...
	needsToWorkingColorSpace(texture) {
		return (
			texture.isVideoTexture === true && texture.colorSpace !== NoColorSpace
		);
	}

	_generateVideoSample(
		textureProperty,
		uvSnippet,
		...
		if (shaderStage === "fragment") {
			...
			console.error(
				`WebGPURenderer: THREE.VideoTexture does not support ${shaderStage} shader.`
			);
		}
	}
	...
	getUniformFromNode(node, type, shaderStage, name = null) {
		...
		if (nodeData.uniformGPU === undefined) {
			...
			if (type === "texture") {
				...
				const bufferClass =
					type === "storageBuffer" ? NodeStorageBuffer : NodeUniformBuffer;
	...
	getBuiltin(name, property, type, shaderStage = this.shaderStage) {
		const map =
			this.builtins[shaderStage] || (this.builtins[shaderStage] = new Map());
	...
	hasBuiltin(name, shaderStage = this.shaderStage) {
		return (
			this.builtins[shaderStage] !== undefined &&
	...
	buildFunctionCode(shaderNode) {
		...
		let code = `fn ${layout.name}( ${parameters.join(", ")} ) -> ${this.getType(
			layout.type
		)} {
${flowData.vars}
${flowData.code}
`;
	...
	enableDirective(name, shaderStage = this.shaderStage) {
		const stage =
			...
			(this.directives[shaderStage] = new Set());
		stage.add(name);
	}
	...
	getVaryings(shaderStage) {
		...
		if (shaderStage === "vertex" || shaderStage === "fragment") {
			...
			for (let index = 0; index < varyings.length; index++) {
				...
				if (varying.needsInterpolation) {
					...
					snippets.push(
						`${attributesSnippet} ${varying.name} : ${this.getType(
							varying.type
						)}`
					);
				} else if (
					shaderStage === "vertex" &&
					vars.includes(varying) === false
				) {
					vars.push(varying);
				}
			}
		...
		return shaderStage === "vertex"
			? this._getWGSLStruct("VaryingsStruct", "\t" + code)
			: code;
	}
	...
	getUniforms(shaderStage) {
		...
		for (const uniform of uniforms) {
			...
			if (uniform.type === "texture") {
				...
				bindingSnippets.push(
					`@binding( ${layout.binding} ) @group( ${layout.group} ) var<uniform> ${uniform.name} : ${vectorType};`
				);
			}
		}
	...
	buildCode() {
		...
		for (const shaderStage in shadersData) {
			...
			const isOutputStruct =
				outputNode !== undefined && outputNode.isOutputStructNode === true;
