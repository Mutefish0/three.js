...
class OperatorNode extends TempNode {
	...
	getNodeType(builder, output) {
		...
		const typeB =
			typeof bNode !== "undefined" ? bNode.getNodeType(builder) : null;
			...
			if (op === "<" && outputLength > 1) {
				if (builder.useComparisonMethod) {
					return builder.format(
						`${builder.getMethod("lessThan", output)}( ${a}, ${b} )`,
						type,
						output
					);
				} else {
				...
				if (builder.useComparisonMethod) {
					return builder.format(
						`${builder.getMethod("lessThanEqual", output)}( ${a}, ${b} )`,
						type,
						output
					);
				} else {
				...
				if (builder.useComparisonMethod) {
					return builder.format(
						`${builder.getMethod("greaterThan", output)}( ${a}, ${b} )`,
						type,
						output
					);
				} else {
				...
				if (builder.useComparisonMethod) {
					return builder.format(
						`${builder.getMethod("greaterThanEqual", output)}( ${a}, ${b} )`,
						type,
						output
					);
				} else {
