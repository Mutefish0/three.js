...
const shaderNodeHandler = {
	...
	get(node, prop, nodeObj) {
		if (typeof prop === "string" && node[prop] === undefined) {
			...
			} else if (node.structType && node.structType.members[prop]) {
				// member access
				const nodeElement = NodeElements.get("$member");
				return nodeElement(nodeObj, prop);
			} else if (prop === "self") {
				return node;
			} else if (
...
const ShaderNodeProxy = function (
	...
	const assignNode = (node) =>
		nodeObject(settings !== null ? Object.assign(node, settings) : node);
...
class ShaderCallNodeInternal extends Node {
	constructor(shaderNode, inputNodes) {
		...
		if (inputNodes && inputNodes.__proto__ == {}.__proto__) {
			const names = Object.keys(inputNodes).sort();

			names.forEach((name) => {
				params.push(`${name}: ${inputNodes[name].uuid}`);
			});
		}

		this.uuid = `${shaderNode.uuid}(  ${params.join(", ")} )`;
	}
	...
	call(builder) {
		...
		if (shaderNode.layout) {
			...
			const outputNode =
				inputNodes !== null ? jsFunc(inputNodes, builder) : jsFunc(builder);
...
export const defined = (v) =>
	typeof v === "object" && v !== null ? v.value : v; // TODO: remove boolean conversion and defined function
...
export const nodeObject = (val, altType = null) =>
	/* new */ ShaderNodeObject(val, altType);
export const nodeObjects = (val, altType = null) =>
	new ShaderNodeObjects(val, altType);
export const nodeArray = (val, altType = null) =>
...
export const string = (value = "") =>
	nodeObject(new ConstNode(value, "string"));
export const arrayBuffer = (value) =>
	nodeObject(new ConstNode(value, "ArrayBuffer"));
